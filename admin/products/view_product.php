...
if (isset($_GET['id']) && $_GET['id'] > 0) {
	...
	if ($qry->num_rows > 0) {
		...
		echo '<script>alert("ID Producto no válido"); location.replace("./?page=products")</script>';
	}
} else {
	echo '<script>alert("ID Producto es requerido"); location.replace("./?page=products")</script>';
}
?>
<style>
